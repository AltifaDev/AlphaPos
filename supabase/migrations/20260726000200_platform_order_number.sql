-- Manual delivery platform order number (Grab / LINE MAN / etc.)
-- Supports counter-service reconciliation without partner APIs.

BEGIN;

ALTER TABLE public.orders
    ADD COLUMN IF NOT EXISTS platform_order_number VARCHAR(80);

COMMENT ON COLUMN public.orders.platform_order_number IS
    'External order ID from delivery platform (manual entry / paste); not an AlphaPos order number';

CREATE INDEX IF NOT EXISTS idx_orders_platform_order_number
    ON public.orders (merchant_id, platform_order_number)
    WHERE platform_order_number IS NOT NULL
      AND COALESCE(is_deleted, false) = false;

-- Keep create_order_atomic in sync with header metadata
