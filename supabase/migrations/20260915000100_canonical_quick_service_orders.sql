...
-- Counter orders never own a table session. Dine-in orders must point at a
-- real, non-deleted table in the same merchant/branch.
...
COMMENT ON FUNCTION public.normalize_order_service_identity() IS
'Keeps Quick Service tableless and rejects non-existent tables for dine-in orders.';
