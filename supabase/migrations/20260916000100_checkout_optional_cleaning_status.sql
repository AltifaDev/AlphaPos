-- Allow each POS checkout to choose whether a paid table waits for cleaning
-- or becomes vacant immediately. The value travels in p_breakdown so the RPC
-- signature remains backward compatible with already deployed clients.
...
BEGIN
    ...
    IF v_kitchen_required AND EXISTS (
    ...
    IF v_table_status NOT IN ('cleaning', 'vacant') THEN
        RAISE EXCEPTION 'invalid_table_status_after_checkout';
    END IF;
    ...
    IF upper(COALESCE(p_table_number,'QUICK')) <> 'QUICK' THEN
      ...
      UPDATE public.restaurant_tables SET status=v_table_status, updated_at=now() WHERE merchant_id=v_order.merchant_id AND branch_id IS NOT DISTINCT FROM v_order.branch_id AND table_number=p_table_number;
    END IF;
    v_response := jsonb_build_object('status','completed','order_id',p_order_id,'amount',v_total,'idempotency_key',p_idempotency_key,'table_status',v_table_status);
