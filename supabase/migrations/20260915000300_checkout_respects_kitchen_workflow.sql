-- Enforce the merchant's serving-before-payment policy for every client.
-- The POS hides the payment button when required, but the RPC must also guard
-- direct/staff/API callers so the policy cannot be bypassed by another device.
