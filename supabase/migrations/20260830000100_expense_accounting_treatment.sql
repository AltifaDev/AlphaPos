-- Keep hosted Supabase aligned with Database/migrations/035_expense_accounting_treatment.sql.
...
UPDATE public.expenses SET recognition_type = 'fixed_asset'
WHERE is_capex = TRUE AND recognition_type = 'operating_expense';
...
CREATE INDEX IF NOT EXISTS idx_expenses_recognition ON public.expenses (merchant_id, recognition_type, date);
CREATE INDEX IF NOT EXISTS idx_expenses_investment_project ON public.expenses (merchant_id, investment_project) WHERE investment_project IS NOT NULL;
