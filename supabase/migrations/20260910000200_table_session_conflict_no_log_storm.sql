-- A stale optimistic-concurrency write is an expected business outcome, not a
-- PostgreSQL failure. Returning a structured server-wins result prevents old
-- clients from retrying the same conflict indefinitely and flooding analytics.
...
BEGIN
  ...
  RETURN jsonb_build_object('id',v_id,'row_version',v_actual,'conflict',false);
END;
$$;
