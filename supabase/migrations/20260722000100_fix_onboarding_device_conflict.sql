...
-- Make device insert idempotent when the same device UUID is retried for the
-- same onboarding merchant. Cross-merchant reuse is handled by activate-merchant
-- minting a fresh device id before calling this function.
...
BEGIN
    ...
    -- merchants.subscription_status is varchar(50); RETURNS TABLE expects text.
    RETURN QUERY
