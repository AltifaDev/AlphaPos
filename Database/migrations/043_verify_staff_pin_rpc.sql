...
-- Canonical copy of supabase/migrations/20260826000100_verify_staff_pin_rpc.sql.
...
BEGIN
    IF v_active_merchant IS NULL OR p_employee_id IS NULL THEN RETURN FALSE; END IF;
    IF p_pin IS NULL OR p_pin !~ '^[0-9]{4}$' THEN RETURN FALSE; END IF;

    v_started_at := clock_timestamp();
    SELECT e.user_id, e.pin_code INTO v_user_id, v_stored_hash
      FROM public.employees e
     WHERE e.id = p_employee_id AND e.merchant_id = v_active_merchant
       AND e.resigned_at IS NULL LIMIT 1;
    v_employee_lookup_ms := EXTRACT(EPOCH FROM (clock_timestamp() - v_started_at)) * 1000;
    IF NOT FOUND THEN
        RAISE LOG 'verify_staff_pin employee_lookup_ms=% user_lookup_ms=0 employee_found=false', round(v_employee_lookup_ms, 2);
        RETURN FALSE;
    END IF;

    v_started_at := clock_timestamp();
    IF v_user_id IS NOT NULL THEN
        SELECT u.pin_code_hash INTO v_expected FROM public.users u
         WHERE u.id = v_user_id AND u.merchant_id = v_active_merchant
           AND COALESCE(u.is_deleted, FALSE) = FALSE AND COALESCE(u.is_active, TRUE) = TRUE LIMIT 1;
        IF v_expected IS NOT NULL AND v_expected <> '' THEN v_stored_hash := v_expected; END IF;
    END IF;
    ...
    IF v_stored_hash IS NULL OR v_stored_hash = '' THEN RETURN FALSE; END IF;
