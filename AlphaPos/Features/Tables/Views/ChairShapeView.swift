...
extension ChairShapeView.Side {
    init(_ chairSide: DynamicTableLayoutView.ChairSide) {
        switch chairSide {
        case .top:    self = .top
...
#Preview {
    VStack(spacing: 20) {
        HStack(spacing: 20) {
            ChairShapeView(side: .top,    color: .teal).frame(width: 32, height: 18)
            ChairShapeView(side: .bottom, color: .teal).frame(width: 32, height: 18)
        }
        HStack(spacing: 20) {
            ChairShapeView(side: .left,  color: .pink).frame(width: 18, height: 32)
            ChairShapeView(side: .right, color: .pink).frame(width: 18, height: 32)
        }
    }
    .padding()
    .background(.white)
